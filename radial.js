...
const PASSIVE_DROPS = [
  ...
  // trophies, then bonemeal, then the rest, all in registry order).
  ...TROPHIES.slice(0, 6).map(t => ({ type: t.id + 's', icon: t.icon, label: t.label, key: t.id + 's' })),
  { type: 'bonemeal',icon: '🧂', label: 'Bone Meal', key: 'bonemeal' },
  ...TROPHIES.slice(6).map(t => ({ type: t.id + 's', icon: t.icon, label: t.label, key: t.id + 's' })),
