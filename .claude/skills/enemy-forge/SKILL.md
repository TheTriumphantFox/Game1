...
5e's CR table assumes a party of four adventurers. Hero of Stormdrift is one player in real-time action combat with no AC/to-hit axis at all — the raw table numbers are wildly too tanky pasted in unchanged, and half the table (AC, attack bonus) has nothing to map onto in the first place.
...
**There is no blight/corrupted system in the code yet.** The Withering Crown and the blight are `hero-of-stormdrift`'s `story-bible.md` canon — design intent, not a built system. If the user asks for a corrupted variant, treat it as new work, not an extension of something that already exists; don't imply otherwise.
