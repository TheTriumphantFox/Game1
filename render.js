...
function render() {
  ...
  // Colossal-tree overlays — like the whirlpool pass, drawn after the tile grid
  // (so neighbours can't overdraw them) and before the fog (so the fog clips any
  // overflow into unexplored tiles). The scan range is widened a few tiles below /
  // either side of the viewport so a giant whose anchor sits just off-screen still
  // pokes its canopy into view. Only drawn once the anchor tile is unfogged.
