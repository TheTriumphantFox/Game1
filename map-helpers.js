...
// There was a faceLedgeEdges here: a pass that DISCOVERED a non-rectangular
// shelf's perimeter from the tiles and turned every LEDGE touching non-shelf into
// LEDGE_FACE. Its only caller was the desert mesa, whose top had to flow around
// chests, shrines, water and its own climb corridors and so could never be a
// rect. Mesa tops are solid T.PLATEAU again (addDesertPlateau, mapgen-biomes.js),
// so the only shelves left in the game are the rectangles stampLedgeShelf lays
// down — and those are faced by construction, which is the safe case and always
