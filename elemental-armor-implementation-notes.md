...
- The Earth-region frog foreshadowing NPC is BUILT (see Tier 4), and the Sovereign now **reads `frog_warned_shadow`** (wired 2026-09-01, clearing the lint note it had been raising every run).
  - Warned: the fight opens with a callback and nothing more. They were told; being told twice is worse than once.
  - Unwarned: the mechanic is stated outright at the open, and stated bluntly again after 22s if they are still fighting on the layout it read. The frog stands on a sealed Earth dead-end eight regions earlier and is **deliberately missable**, so this cannot be the payoff to a setup half the players never saw — the alternative is a tier-12 boss unbeatable by anyone who skipped an optional NPC on an optional map.
  - The nudge fires once, never for the warned, never once the player has already changed something, and never while the Sovereign is already blinded. Verified.
