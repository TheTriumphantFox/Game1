...
function addGreatTrees(m, regionId, depth) {
  if (regionId !== 'mana') return;
  const isOpen = (r, c) => {
    const t = m[r][c];
...
// grid — overhangs into view rather than being buried off-screen. Sporadic (1d4+2)
// and well-spaced (≥22-tile Manhattan gap), kept clear of the exit gates. Runs after
// the connectivity seal — COLOSSAL_TREE only ever replaces an already-solid border
// tile (solid → solid), so the passable graph is never affected. No-op elsewhere.
function addColossalTrees(m, regionId) {
  if (regionId !== 'mana') return;
  ...
  const want = rnd(1, 4) + 2;   // sporadic — 3..6 per map
  const MIN_GAP = 22;
  const placed = [];
  for (const [r, c] of cands) {
    if (placed.length >= want) break;
...
function buildVillageMap(biome) {
  ...
  addGreatTrees(m, regionId, 8);
  addColossalTrees(m, regionId);
